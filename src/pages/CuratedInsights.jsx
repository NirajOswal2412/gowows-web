...
function CuratedInsights() {
  ...
  const fetchInsights = async () => {
    try {
      const res = await secureFetch(`${BASE_URL}/curated-insights`);
      if (!res) return;
      const data = await res.json();
      setInsights(data);
      setOutlineText(generateOutlineFromInsights(data));
    } catch (err) {
  ...
  const exportToExcel = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(`${BASE_URL}/export-outline-excel`, {
