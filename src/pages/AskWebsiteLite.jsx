...
function AskWebsiteLite() {
  ...
  const smartPrompts = getSmartPrompts("askweb");
  const outlineText = getOutlineText("askweb");
  ...
  const handleAsk = async () => {
    ...
    try {
      const res = await secureFetch(`${BASE_URL}/ask-web-lite`, {
        method: "POST",
  ...
  const fetchPrompts = async (response) => {
    try {
      ...
      if (!res) return;
      const data = await res.json();
      setSmartPrompts("askweb", data.prompts || []);
    } catch (err) {
      console.error("❌ Error fetching prompts:", err);
      setSmartPrompts("askweb", []);
    }
  };
  ...
  const generateOutline = async () => {
    try {
      ...
      if (!res) return;
      const data = await res.json();
      setOutlineText("askweb", data.outline);
