...
// Removed broken CDN worker assignment
...
function AskPDF() {
  ...
  const handleAsk = async () => {
    ...
    try {
      ...
      fetchPrompts(res.data.response);
    } catch (err) {
  ...
  const fetchPrompts = async (text) => {
    try {
      console.log("📤 Sending to /generate-prompts:", { text });
      const res = await axios.post("http://localhost:7860/generate-prompts", { text });
  ...
  const handleReadEntirePDF = async () => {
    if (!pdfFile) return;
    ...
    fileReader.onload = async function () {
      try {
        ...
        console.error("❌ Error reading full PDF:", err);
      }
    };
